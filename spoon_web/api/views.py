...
def fetch_stale(request):
    m = Manager(database=redis)
    px_kv = m.scan_kv_from("spoon:proxy_stale", random.randint(0, 1000000))
    res_list_pre = [[k.decode('utf-8'), float(v.decode('utf-8'))] for (k, v) in px_kv[1].items()]
    res_list = [k[0] for k in res_list_pre]
